...
export default async function AdminDashboard() {
  const [{data: properties}, {data: inspections}, {data: users}] =
    await Promise.all([
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="card">
          ...
          <Link href="/properties" className="card-link">Manage</Link>
        </div>

        <div className="card">
          <div className="text-4xl font-bold">{inspections?.length}</div>
          <div className="text-gray-500">Inspections</div>
          <Link href="/inspections" className="card-link">View All</Link>
        </div>

        <div className="card">
          <div className="text-4xl font-bold">{users?.length}</div>
          <div className="text-gray-500">Users</div>
          <Link href="/users" className="card-link">Manage</Link>
        </div>
      </div>
    </div>
  );
